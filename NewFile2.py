...
class Pharma:
    ...
    def add(self,name,s):
        ...
        query2 = "SELECT price FROM bigtable WHERE productName=(%s)"
        self.cursor.execute(query2, (name2,))
        myresultp = self.cursor.fetchall()
        ...
        for row in myresultp:
            dictio3.append(row[0])
            self.printprice(dictio3[0:1])




    def remove(self,name,s):
        x3=s.get()
        name3=name
        query112 = "UPDATE bigtable SET Available=Available-1 WHERE ProductName=(%s) AND PharmaName=(%s)"
        self.cursor.execute(query112, (name3,x3))
        #query1 = "UPDATE (%s) AS pName SET pName.Available=bigtable.Available FROM bigtable INNER JOIN pName ON (pName.ProductName=bigtable.ProductName)"
        #self.cursor.execute(query1, (name2,))
        query2 = "SELECT price FROM bigtable WHERE productName=(%s)"
        self.cursor.execute(query2, (name3,))
        myresultp = self.cursor.fetchall()
        ...
        for row in myresultp:
            dictio3.append(row[0])
            price3=dictio3[0:1]
            self.prices=self.prices-price3



    def printprice(self, totals):
        totals = totals
        top = tk.Tk()

        top.geometry("100x100")

        messagebox.showinfo("total price", totals)
    ...
    def login(self):
            ...
            buttonCal = tk.Button(root2, text="login", command=partial(self.login2, e, p, name_var, passw_var))

            buttonCal.grid(row=3, column=0)
            root2.mainloop()

    def login2(self, e, p, x, y):
            ep = e.get()
            pe = p.get()
    ...
    def search(self,s):
        ...
        tv.heading(2, text='available')
        tv.heading(3, text='price')
        tv.heading(4, text='location')
        tv.heading(5, text='pharmaname')


        print(dictio)

        tv.insert(parent='', index=0, iid=0, text='',values=(dictio[0:1],dictio[1:2],dictio[2:3],dictio[3:4],dictio[4:5]))
        ...
        #add_but3 = ttk.Button(ws, text="done", command=partial(self.printprice,r))
...
try:
        connection = mysql.connector.connect(host="localhost", user="root", password="Sazia12345Shahreen",
